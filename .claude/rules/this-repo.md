...
September 8, 2026). Via [`.claude/settings.json`](../settings.json) all six are switched on —
...
orchestrator's body (imported via the root `CLAUDE.md`) advances on that refresh alone and everything else waits for the cut. Between
...
[the system-administration lens](../specialists/lenses/specialist-05-15-lens.md#repo-specific-rules).
...
sentence above stays true, because CI is not a session; the trap is that the paragraph reads as the
whole propagation model, so a reader reasoning from it concludes
...
(see the roster in [`SPECIALISTS.md`](../specialists/SPECIALISTS.md)); its portable law pages now state that reach per chapter -- three repos for
...
[`SPECIALISTS.md`](../specialists/SPECIALISTS.md) says so without exception — so eleven of them hold
...
[`language-layers.md`](language-layers.md).** It is path-scoped to
...
[`language-layers.md`](language-layers.md) already makes.
...
[Tessa's](../specialists/lenses/specialist-06-16-lens.md) records what the pages were, what the
...
- **The main branch is `main`.** All changes via a `<prefix>/<short-name>` branch + PR to
  ...
  12 times. See
  [the branch-taxonomy lens](../specialists/lenses/specialist-05-05-lens.md#classifying-naming-and-creating-a-branch).
- **The lint and test gates are the safety guard before every PR.**
  ...
  no PR is opened (`-SkipLint`/`-SkipTests` are the escape valves). See [the system-administration lens](../specialists/lenses/specialist-05-15-lens.md).

  **Why these checks have the shape they do — and every rule that was measured and DECLINED — is in
  [the system-administration lens](../specialists/lenses/specialist-05-15-lens.md#how-the-gate-checks-got-their-shape-and-the-measurements-behind-them-august-15-2026)**:
...
- **More gates arrive with the workflow plugin**, and all of them read the branch's own document. Most run
  ...
  **DEPLOY lock** refuses to merge once that section no longer matches what the PR published — it is fixed
  ...
  [Sylvester's lens](../specialists/lenses/specialist-05-15-lens.md).
- **The staleness race is answered by detect-and-rebase, and a merge queue is no longer the policy**
  ...
  [Sylvester's lens](../specialists/lenses/specialist-05-15-lens.md) and the writing lesson in
  [Tessa's](../specialists/lenses/specialist-06-16-lens.md).
- **And one guard fires *after* the merge, on the trunk.** The fold runs from `ship-pr.ps1`, as the
  ...
  **Under a queue `ship-pr.ps1` no longer even tries** ([#1506](https://github.com/DKJ-Solutions/claude-code-specialists/issues/1506)):
  ...
  [his lens](../specialists/lenses/specialist-05-15-lens.md#what-sylvester-owns-here).
...
alone and sits in this slot. The specialist roster is one `@`-import in the root `CLAUDE.md`.
