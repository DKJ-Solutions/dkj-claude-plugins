...
| `lib/fence-lib.ps1` | `Get-NextFenceState` — the one fence tracker every markdown reader here walks with, CommonMark-style: a block closes only on a run of the same character at least as long as its opener, so a quoted example that itself carries a fence stays quoted (issue #2536) | none — dot-sourced lib |
