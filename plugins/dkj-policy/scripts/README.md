...
| `task/adopt-ci-floor.ps1` | places the two CI runners that keep the fold and the resolves verification alive across a merge the shipping session never observes, reports whether a required status check exists at all, and — only where a merge queue is actually active — reports its prerequisite and prints the ruleset command without running it | [`adopt-dkj-policy`](../skills/adopt-dkj-policy/SKILL.md) (Part 3) |
