...
$script:ContractRecords = @(
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-TriageLabels'; Scripts = @('adopt-triage-labels');
       ...
       Optional = $true; Default = "the same five labels, built into adopt-triage-labels.ps1 as its own fallback -- a consumer who has never answered this seam is already told the canonical set rather than a degraded one";
       Returns = "an array of objects with Name, Color and Description -- the canonical triage labels: the four priority rungs 'prio-1' (lowest) through 'prio-4' (highest), plus 'dossier', the kind label for a collecting issue no single repair closes (#2462) -- exactly the fields a 'gh label create' call needs. NOT a gate: nothing in this workflow refuses a PR or a merge over a missing triage label, unlike the reach label whose absence a consumer's own 'gh issue create' call fails on -- this seam exists only so adopt-triage-labels.ps1 has something authoritative to compose its paste-ready commands from" },
