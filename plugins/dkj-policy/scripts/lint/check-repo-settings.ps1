...
.PARAMETER Trunk
    The branch whose effective rules are read. An explicit value here always wins; otherwise this
    reads the optional Get-TrunkBranchName seam in scripts/repo-config.ps1 -- the same seam every
    other portable script in this tree reads (e.g. entry-scaffold-lib.ps1's Get-BranchTrunkName) --
    and falls back to 'main' when that seam is absent too.
...
# IT IS NOW A PLUGIN MIRROR (issue #1843) -- WHICH REVERSES WHAT THIS COMMENT SAID UNTIL NOW ("this
# script is repo-local, and the registry in shared-scripts-lib.ps1 is deliberately not extended").
# Amended rather than silently flipped, so both readings stay legible. That registry entry was declined
# on the ground that "#1726 asked about this repo": #1726 measured THIS repo's own drift and never asked
# what a consumer should run, so extending the registry read as scope creep at the time. #1843 asked the
# wider question directly, and Dave's scope decision on it answered it: "the reachable goal is identical
# scripts available, not identical rules enforced" (September 12, 2026) -- a consumer's GitHub-side state
# drifts exactly the same way this repo's did, with nothing in their own tree saying so. THE MOVE COSTS
# NOTHING BECAUSE THE VALUES WERE ALREADY BEHIND A SEAM: what travels is this script, which reads
# Get-ExpectedRepoSettings; the DECLARATION stays 'decide' rather than 'copy' in
# scripts/lib/script-contract-lib.ps1, exactly because what counts as drift is a fact about the
# consumer's own ruleset, not this repo's.

# AND BECOMING A MIRROR IS WHY THE SOURCE-REPO GUARD JOINS IT NOW, where before there was nothing to
# guard: a repo-local script has no released copy to run stale, and a mirrored one does. Two callers
# read it -- a scheduled workflow (in this repo, the in-repo path; in a consumer, a second checkout of
# the plugin tree) and a person running the command in this file's own .SYNOPSIS -- neither is a
# SessionStart or Stop hook, which is the ONLY reason this repo's own guard-coverage suite
# (source-repo-guard.tests.ps1) accepts for leaving a shared entry point unguarded. So this does not
# join that exemption list; it carries the guard like an ordinary shared script.
...
# THE TRUNK COMES FROM THE SEAM WHEN NOT GIVEN EXPLICITLY (issue #1843). Without this, a consumer on
# 'master' has every branch-rules read aimed at a branch it does not have, and -RequireRead then reports
# a red run whose real cause -- the wrong branch name, not an unreadable token -- is never named.
# $PSBoundParameters IS WHAT TELLS "THE CALLER SAID SO" APART FROM "THAT IS JUST THE DEFAULT": $Trunk's
# own default already reads 'main', so checking the value would never be able to tell an explicit
# '-Trunk main' apart from no flag at all, and the explicit value has to keep winning either way.
if (-not $PSBoundParameters.ContainsKey('Trunk') -and (Test-FunctionDefined -Name 'Get-TrunkBranchName')) {
    $seamTrunk = Get-TrunkBranchName
    if ($seamTrunk) { $Trunk = [string]$seamTrunk }
}
