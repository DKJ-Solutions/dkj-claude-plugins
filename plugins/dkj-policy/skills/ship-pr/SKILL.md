...
| `-BypassNote` | Passed through to `open-pr`: why `-SkipLint`/`-SkipTests` was used. Every run that skips a gate writes a **Gate bypass** section into the PR body; this is the reason it gives. Survives `-RefreshBody`. |
