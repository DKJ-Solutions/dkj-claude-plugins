...
if ($install.Records.Count -gt 0) {
    ...
    $notes += "No install record points at this repo any more, so there is nothing left to uninstall here. Checked on projectPath against $($install.Path) -- the same query the install verification in plugins/ADOPTION.md runs. If you are re-running this audit after the uninstall, this line IS the expected reading, not a step you skipped."
} else {
