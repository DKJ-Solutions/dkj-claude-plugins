...
$script:ContractRecords = @(
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReachLabel'; Scripts = @('report-issue skill', 'adopt-dkj-policy skill', 'build-backlog-page skill', 'guard-asana-mirror hook');
    ...
    # THE CLOSE-OUT BAND (issue #2050). 'decide', and it is the clearest 'decide' in the table for a
    # reason that has nothing to do with what a repo IS. Every other Adopt = 'decide' record is about a
    # FACT the consumer alone can state -- where their notes live, what their branch prefixes mean. This
    # one is about CONSENT: answering it arms a hook that refuses to let a turn end, and a value copied
    # in by an adoption run would arm it in a repo that never weighed the trade. Copying it is the one
    # thing this seam must not do, which is also why the resolver treats an absent AND a malformed answer
    # as off rather than falling back to a band.
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-CloseOutGateBand'; Scripts = @('closeout-gate');
       Adopt = 'decide'; AdoptWhy = "answering it turns on a Stop hook that BLOCKS a turn, which is a thing to opt into rather than to inherit -- so adopt-config never places it and a repo that has not weighed it keeps the behaviour it had before the gate existed. Read scripts/maintenance/measure-closeouts.ps1 against your own transcripts before picking a number: the source repo's 6 came from ITS distribution (over 6 = 27% of close-outs there), and a repo whose close-outs are shorter or longer wants a different band from the same reasoning";
       ViaLib = 'closeout-gate-lib';
       Optional = $true; Default = "no gate at all. Unlike Get-AlwaysOnBudget, an unanswered repo does NOT fall back to a built-in band -- a ceiling on a document is worth defaulting, a hook that blocks a turn is not. A non-numeric or non-positive answer is off for the same reason, so 0 is a real answer: it switches the gate off without deleting the function";
       Returns = "the maximum number of NON-EMPTY lines a close-out may run to before the Stop hook refuses the turn and asks for it again. Blank separators are excluded deliberately, so a receipt cannot pass or fail on its formatting. The band is an INCLUSIVE upper bound -- a close-out of exactly the band passes" },
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-TriageLabels'; Scripts = @('adopt-triage-labels');
       ...
       Optional = $true; Default = "the same six labels, built into adopt-triage-labels.ps1 as its own fallback -- a consumer who has never answered this seam is already told the canonical set rather than a degraded one";
       Returns = "an array of objects with Name, Color and Description -- the canonical triage labels: the four priority rungs 'prio-1' (lowest) through 'prio-4' (highest), plus 'dossier', the kind label for a collecting issue no single repair closes (#2462), and 'needs-decision', the parking label for an issue awaiting the owner's choice (#2519) -- exactly the fields a 'gh label create' call needs. NOT a gate: nothing in this workflow refuses a PR or a merge over a missing triage label, unlike the reach label whose absence a consumer's own 'gh issue create' call fails on -- this seam exists only so adopt-triage-labels.ps1 has something authoritative to compose its paste-ready commands from" },
