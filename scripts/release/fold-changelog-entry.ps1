...
foreach ($file in $entryFiles) {
    ...
    if ($prs.Count -ge 1) {
        ...
        # it, since August 23, 2026 (it sat on the 'Pull Request' heading before that) -- the counterpart
        # of the creation stamp the cycle file's heading carries -- and the closing line then carries only
        # the link, so one fact stands in one place.
        #
        # UNLESS THE ENTRY HAS NO SUCH HEADING, which was true of a pre-dossier entry (title AS heading,
        # no named sections) before this test predates it. See #2259 for where that guarantee has since
        # drifted: since August 26, 2026 a pre-dossier entry's re-levelled heading and today's entry
        # heading share a level, so Set-EntryMergeStamp finds one to stamp there after all, while this test
        # still reads the pre-August-23 signal ('PullRequest' section presence) to decide the fallback.
        # Test-EntryHasSection is the same reader the emptiness gate uses for "absent versus empty", so the
        # two cannot disagree about which shape they are looking at.
