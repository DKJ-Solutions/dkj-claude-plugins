...
# The shared triage labels (issue #1895; 'dossier' added by #2462, 'needs-decision' by #2519), Adopt='copy' in the script contract -- this repo's
# own live answer, and the same six values adopt-triage-labels.ps1 carries as its own built-in
...
Assert-Equal 6 $triageLabels.Count 'Get-TriageLabels names exactly six labels -- four rungs, the dossier kind and the needs-decision parking label'
...
Assert-Equal 'prio-1,prio-2,prio-3,prio-4,dossier,needs-decision' (($triageLabels | ForEach-Object { $_.Name }) -join ',') `
    'Get-TriageLabels names prio-1 through prio-4, then dossier, then needs-decision, in that order'
